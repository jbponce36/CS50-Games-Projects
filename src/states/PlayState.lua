...
function PlayState:init()
    ...
    -- flag for rendering Board resetted label
    self.resettingBoard = false

     -- start our Board resetted label off-screen
    self.resettingLabelY = -64
...
function PlayState:enter(params)
    ...
    -- reset the board until there is at least one match possible
    while not self.board:availableMatches() do
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            --for every tile in a match, add corresponding score
    ...
    -- if no matches, swap back the last swapped tile
    else
...
--[[
    Calculates the if there are available matches left and resets the board if there aren't any
]]
function PlayState:checkAvailableMatches()
    ...
    if not self.board:availableMatches() then
        ...
        :finish(function()
            Timer.tween(0.25, {
                [self] = {resettingLabelY = VIRTUAL_HEIGHT / 2 - 8}
            })
            ...
            :finish(function()
                Timer.after(1, function()
                    ...
                    Timer.tween(0.25, {
                        [self] = {resettingLabelY = VIRTUAL_HEIGHT + 30}
                    })
                    :finish(function() 
                        self.resettingBoard = false
                    end)
...
function PlayState:render()
    ...
    if self.resettingBoard then
        -- render Board resetted... label and background rect
        love.graphics.setColor(95/255, 205/255, 228/255, 200/255)
        love.graphics.rectangle('fill', 0, self.resettingLabelY - 8, VIRTUAL_WIDTH, 48)
        love.graphics.setColor(1, 1, 1, 1)
        love.graphics.setFont(gFonts['large'])
        love.graphics.printf('Board resetted... ', 0, self.resettingLabelY, VIRTUAL_WIDTH, 'center')
