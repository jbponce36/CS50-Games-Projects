...
function PlayState:setPlayerPosition()
    local column = 1
    local isColumnEmpty = self.tileMap:isColumnEmpty(column)
    while isColumnEmpty do
        column = column + 1
        isColumnEmpty = self.tileMap:isColumnEmpty(column)
    end
    self.player.x = (column - 1) * TILE_SIZE
end
