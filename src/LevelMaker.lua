...
function LevelMaker.createMap(level)
    ...
    -- counts the locked bricks the level currently has
    gLockedBricks = 0

    -- flag for checking if a level had locked bricks when it initiated
    gLevelHadLockedBricks = false

    local lockedBrickColor = 6
    local lockedBrickTier = 1

    -- 50/50 chance to generate a level containing locked bricks
    local generateLockedBricks = math.random(0, 1)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- chance for spawning a locked brick
            local chanceForLockedBrick = math.random(1, 10)
            
            -- if chance rolled on 1, don't generate locked bricks for this level
            if generateLockedBricks == 1 then
                chanceForLockedBrick = 0
            end
            
            -- spawn a locked brick with 20 per cent chance
            if chanceForLockedBrick > 8 then
                gLockedBricks = gLockedBricks + 1
                gLevelHadLockedBricks = true

                b.color = lockedBrickColor
                b.tier = lockedBrickTier
            else
