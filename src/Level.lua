...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        -- if there is only one alien
        if #self.launchMarker.splittedAliens == 0 then
        ...
        -- if we splitted the alien in 3
        else
            local xPos1, yPos1 = self.launchMarker.splittedAliens[1].body:getPosition()
            local xVel1, yVel1 = self.launchMarker.splittedAliens[1].body:getLinearVelocity()

            local xPos2, yPos2 = self.launchMarker.splittedAliens[2].body:getPosition()
            local xVel2, yVel2 = self.launchMarker.splittedAliens[2].body:getLinearVelocity()

            -- check the position and velocity of all 3 of them
            if (xPos < 0 and xPos1 < 0 and xPos2 < 0) or    
                ((math.abs(xVel) + math.abs(yVel) < 1.5) and 
                (math.abs(xVel1) + math.abs(yVel1) < 1.5) and 
                (math.abs(xVel2) + math.abs(yVel2) < 1.5)) then
            
                self.launchMarker.alien.body:destroy()
                self.launchMarker.splittedAliens[1].body:destroy()
                self.launchMarker.splittedAliens[2].body:destroy()
