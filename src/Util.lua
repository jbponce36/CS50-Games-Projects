...
function GenerateQuadsBricks(atlas)
    -- subset of quads that contains regular bricks
    local bricks = table.slice(GenerateQuads(atlas, 32, 16), 1, 21)

    -- subset of quads that contains the locked brick
    local lockedBrick = table.slice(GenerateQuads(atlas, 32, 16), 24, 24)

    -- append the locked brick to the regular bricks table
...
function GenerateQuadsPowerups(atlas)
    return table.slice(GenerateQuads(atlas, 16, 16), 12*12+1, 12*12+1+10)
end