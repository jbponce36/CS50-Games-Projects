...
function AlienLaunchMarker:init(world)
    ...
    -- whether the original alien collided with anything yet
    self.collided = false
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        -- if we launched the original alien already and it did not collide with anything yet
        if not self.collided then
            if love.keyboard.isDown('space') and #self.splittedAliens == 0 then
                gSounds['split']:stop()
                gSounds['split']:play()
                ...
                local alien1 = Alien(
                    self.world, 
                    'round', 
                    math.floor(self.alien.body:getX()), 
                    math.floor(self.alien.body:getY() - 32), 
                    'Player'
                )

                -- rotate the velocity vector -10 degrees using a rotation matrix
                alien1.body:setLinearVelocity(
                    0.98 * velX + 0.17 * velY, 
                    (-0.17) * velX + 0.98 * velY)
                ...
                table.insert(self.splittedAliens, alien1)
                table.insert(self.splittedAliens, alien2)
            end
        end
    end
end
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        for k, splittedAlien in pairs(self.splittedAliens) do
            splittedAlien:render()
        end
    end
end